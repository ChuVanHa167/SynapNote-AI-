...
async def upload_audio(
    ...
    public_url = f"/uploads/{audio_filename}"

    # 5. Save and convert file
    ...
    try:
        if is_video:
            ...
            print(f"[Upload] Converting video to audio: {audio_path}")
    ...
    # 6. Update DB with audio URL
    updated_meeting = service.meeting_repo.update(new_meeting.id, {"audio_url": public_url})

    # 7. Start background AI processing
    ...
    print(f"[Upload] Returning meeting: {updated_meeting.id if updated_meeting else 'None'}")
