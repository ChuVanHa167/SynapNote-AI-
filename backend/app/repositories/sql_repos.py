...
class SqlMeetingRepository(IMeetingRepository):
    ...
    def _to_schema(self, db_meeting: models.Meeting) -> schemas.Meeting:
        # Map ORM to Pydantic - handle relationships safely
        try:
            action_items = [
                schemas.ActionItem(
                    id=ai.id,
                    ...
                    status=ai.status
                ) for ai in (db_meeting.action_items or [])
            ]
        except Exception:
            action_items = []

        try:
            decisions = [d.content for d in (db_meeting.decisions or [])]
        except Exception:
            decisions = []
        ...
        return schemas.Meeting(
            ...
            participants=db_meeting.participants or 1,
            date=db_meeting.date,
            duration=db_meeting.duration or "0m 0s",
            status=db_meeting.status or "PENDING",
